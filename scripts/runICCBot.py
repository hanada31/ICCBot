...
def analyzeApk(apkPath, resPath, sdk):
    ...
    if(os.path.exists(apkPath)): 
        ...
        for apk in apks:
            if apk[-4:] ==".apk":
                print("java -jar "+jarFile+"  -path "+ apkPath +" -name "+apk+" -androidJar "+ sdk +"/platforms  "+ extraArgs +" -time 30 -maxPathNumber 100 -client ICCSpecClient  -outputDir "+outputDir+" >> "+logDir+"/"+apk[:-4]+".txt")
                os.system("java -jar "+jarFile+"  -path "+ apkPath +" -name "+apk+" -androidJar "+ sdk +"/platforms "+ extraArgs +" -time 60 -maxPathNumber 100 -client ICCSpecClient -outputDir "+outputDir+" >> "+logDir+"/"+apk[:-4]+".txt")
